use super::super::traits::{ByteEncodable, ByteDecodable};
use super::super::errors::{ByteVecError, BVExpectedSize};
use super::super::{BVEncodeResult, BVDecodeResult, BVSize};
    ...
    ($t:ident: $elem:ident) => {
            ...
            where $t: ByteEncodable
        {
            ...
            fn encode<Size>(&self) -> BVEncodeResult<Vec<u8>> where Size: BVSize + ByteEncodable {
                if self.get_size::<Size>().is_some() {
                    let mut bytes = Vec::new();
                    bytes.extend_from_slice(&self.0.get_size::<Size>().unwrap().encode::<Size>()?);
            ...
            where $t: ByteEncodable, $($_t: ByteEncodable),*
        {
            ...
            fn encode<Size>(&self) -> BVEncodeResult<Vec<u8>> where Size: BVSize + ByteEncodable {
                if self.get_size::<Size>().is_some() {
                    ...
                    bytes.extend_from_slice(&$elem.get_size::<Size>().unwrap().encode::<Size>()?);
                    $(
                        bytes.extend_from_slice(&$_elem.get_size::<Size>().unwrap().encode::<Size>()?);
                    )*
                    bytes.extend_from_slice(&$elem.encode::<Size>()?);
                    $(
